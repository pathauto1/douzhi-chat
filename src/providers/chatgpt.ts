...
const SELECTORS = {
  composer:
    '#prompt-textarea, [data-testid="composer-input"], div[contenteditable="true"][id="prompt-textarea"]',
...
export const chatgptActions: ProviderActions = {
  async isLoggedIn(page: Page): Promise<boolean> {
    try {
      // Wait briefly for either state to settle
      await page.waitForTimeout(2000);
      const composer = await page.$(SELECTORS.composer);
      if (composer) return true;

      // Explicitly check for login page indicators
      const loginButton = await page.$(SELECTORS.loginPage);
      if (loginButton) return false;

      // Neither found — page may still be loading; assume not logged in
      return false;
    } catch {
      return false;
    }
